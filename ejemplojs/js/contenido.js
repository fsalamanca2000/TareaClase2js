...
function imprimir(){
    ...
    function nuevoEnlace(separador) {
        var contenido = document.getElementById('contenido');
        var texto = document.getElementById("texto").value;
        var separador = ",";
        var partes = texto.split(separador);
        if (partes.length >= 2) {
            var textoAntes = partes[0].trim();
            var enlace = partes[1].trim(); 
            var enlaceHTML = `<a href="${enlace}" target="_blank">${textoAntes}</a>`;
            contenido.innerHTML += enlaceHTML;
        } else {
            contenido.innerHTML = 'Por favor, introduce el texto seguido de una coma y un enlace.';
        }
    }
